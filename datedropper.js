...
$.fn.dateDropper = function(options) {
  ...
  if (this.is('input') && this.attr('type') == "text") {
    ...
    var
      ...
      yranger = function(yr) {
        return yr.toString().substr(0, 3) + settings.years_multiple;
      },
      ymultiselect = 0;
    ...
    if (settings.default_date) {
      input.val(settings.default_date);
    }
...
var
  ...
  setDateAnimate = function() {
    month.find('.dd_sl_').animate({
      scrollLeft: month.find('li.dd_sltd_').index() * range
    }, 1200, 'swing');
    setTimeout(function() {
      day.find('.dd_sl_').animate({
        scrollLeft: day.find('li.dd_sltd_').index() * range
      }, 1200, 'swing');
      setTimeout(function() {
        year.find('.dd_sl_').animate({
          scrollLeft: year.find('li.dd_sltd_').index() * range
        }, 1200, 'swing');
      }, 200);
    }, 400);
  },
...
if (!tempD && !tempM && !tempY) selectCurrent();
else setValueDate();
...
var
  calc = function() {
    ...
    day.find('li').each(function(index, element) {
      ...
      if (x == 0) $(this).addClass('dd_sunday');
      else $(this).removeClass('dd_sunday');
...
if (settings.years_multiple) {
  ...
  year_r.find('li').on('click', function() {
    ...
    year.find('.dd_sl_').stop().animate({
      scrollLeft: (year.find('li[value=' + x + ']').index()) * range
    }, 1200, 'swing');
